...
class adminStatusOnChannelComm{

    public function start($ts, $config, $client, $lang)
    {
        preg_match('/((!adminStatusOnChannel)(.)(help|list))|((!adminStatusOnChannel)(.)((add)|(remove))(.)([0-9]*)(.)([0-9]*))|((!adminStatusOnChannel))/', $client['msg'], $command);
        if (strstr($client['msg'], "!adminStatusOnChannel") !== false) {
            if (array_intersect(qBot::getGroupsFromId($ts, $client['invokerid']), $config['allowedGroups'])) {
                if (@$command[9] == 'add' && !empty($command[12]) && !empty($command[14]) && empty($command[4])) {
                    ...
                    if (empty($data[$command[12]])) {
                        ...
                        file_put_contents("cache/adminStatusOnChannel.json", json_encode($data));
